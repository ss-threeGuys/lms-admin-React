...
export default (hocName, hocTarget, hocColumnMap) => {
    ...
    renderForm() {
      let footer = (
        <div className="ui-dialog-buttonpane p-clearfix">
          ...
          {this.state.selected ? (
            <Button
              label={this.state.deleteConfirmed ? "Confirm Delete" : "Delete"}
              icon="pi pi-times"
      ...
      return (
        ...
        >
          <div className="p-grid p-fluid">{this.renderFormInput()}</div>
        </Dialog>
      );
    }
      ...
      return (
        <div>
          <DataTable
            ...
            totalRecords={
              this.props.paging?.totalRecords ||
              this.defProps.paging.totalRecords
            }
            onSort={this.onSort.bind(this)}
            sortField={
              this.props.sorting?.sortField || this.defProps.sorting.sortField
            }
            sortOrder={Number(
              this.props.sorting?.sortOrder || this.defProps.sorting.sortOrder
            )}
          >
            {this.renderColumn()}
          </DataTable>
  ...
  function mapDispatchToProp(dispatch, props) {
    return {
      set: newProps => {
        dispatch(doTask(hocTarget, Task.SETPROPS)(newProps));
      }
    };
  }
