import columnMap from '../../domains/publisher.map'
...
export default function publisherReducer(state = initialState, action) {
    log.debug('publisherReducer(state = initialState, action)', state, action);
    
    if (action.target !== Target.PUBLISHER) // Not my job
        return state;
        ...
        case Task.RETRIEVE:
            ...
            if (action.actionState === State.START ) {
                ...
                newState.props.paging = {
                    first: (action.payload.paging.currentPage-1)*action.payload.paging.pageSize,
